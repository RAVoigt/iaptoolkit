...
class GoogleIAPKeys:
    ...
    def refresh(self):
        ...
        if not certs:
            raise GooglePublicKeyException(f"Failed to retrieve JSON public keys from Google at: '{GOOGLE_IAP_PUBLIC_KEY_URL}'")
    ...
    def should_refresh(self) -> bool:
        if self._retrieved_timestamp > datetime.datetime.now() - datetime.timedelta(seconds=self._key_ttl_seconds):
            return False
        return True
